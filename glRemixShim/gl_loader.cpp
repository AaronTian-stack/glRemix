...
namespace gl
{
    ...
    void initialize()
    {
        std::call_once(g_initialize_flag, [] {
            ...
            HMODULE h_module;
            if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT, reinterpret_cast<LPCSTR>(&initialize), &h_module) &&
                GetModuleFileNameA(h_module, dll_path.data(), MAX_PATH))
            {
                ...
                *strrchr(dll_path.data(), '\\') = '\0';
                ...
                if (CreateProcessA(nullptr, dll_path.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi))
                {
