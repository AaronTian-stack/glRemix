    ...
    while (offset + sizeof(GLCommandUnifs) <= bytesRead) // while header + additional gl header is not at byte limit
    {
        ...
        switch (header->type)
        {
			...
			case GLCommandType::GL_BEGIN: 
			{
				...
				readGeometry(ipcBuf, offset, vertices, indices, static_cast<GLTopology>(type->mode), bytesRead); // store geometry data in vertex buffers depending on topology type
				break;
			}
            default:
			{
