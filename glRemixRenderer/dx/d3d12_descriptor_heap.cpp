...
bool D3D12DescriptorHeap::create(ID3D12Device* device, const D3D12_DESCRIPTOR_HEAP_DESC& desc)
{
	...
	{
		.Flags = desc.Flags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE ? D3D12MA::VIRTUAL_BLOCK_FLAG_ALGORITHM_LINEAR : D3D12MA::VIRTUAL_BLOCK_FLAG_NONE,
		.Size = desc.NumDescriptors, // Descriptor is atomic unit, not bytes
		.pAllocationCallbacks = nullptr,
	};
	const auto hr = D3D12MA::CreateVirtualBlock(&block_desc, m_virtual_block.ReleaseAndGetAddressOf());
...
void D3D12DescriptorHeap::get_cpu_descriptor(D3D12_CPU_DESCRIPTOR_HANDLE* handle, size_t num_descriptor_offset) const
{
...
bool D3D12DescriptorHeap::get_gpu_descriptor(D3D12_GPU_DESCRIPTOR_HANDLE* handle, size_t num_descriptor_offset) const
{
	if (!(m_desc.Flags & D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE))
	{
		OutputDebugStringA("D3D12 ERROR: Trying to get GPU descriptor handle from a non-shader-visible descriptor heap\n");
		return false;
	}
