...
namespace glRemix
{
    ...
    */
    ...
    public:
        ...
        bool CreateForWriter(const wchar_t* name = kDEFAULT_MAP_NAME, uint32_t capacity = kDEFAULT_CAPACITY);
        ...
        inline uint8_t* GetPayload() const { return m_payload; }
        inline uint32_t GetCapacity() const { return m_header ? m_header->capacity : 0; }

        /* for usage in future sync ops. dummy fxns for now */
        HANDLE WriteEvent() const { return m_writeEvent; }
        HANDLE ReadEvent() const { return m_readEvent; }
